...
def training(args, model, train_loader, test_loader, Loss, data_weight ,optimizer, epochs, device, num_class, checkpoint_name, event, class_threshold=0.5,swa_model=None,pretrained_state=None,fold=None):
    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        if epoch > 1:
            while event.is_set():
                time.sleep(5)
                print('Other edges processing')
            #load aggregate pretrain model
        ...
        if epoch == 1 and not args.use_pretrained :
