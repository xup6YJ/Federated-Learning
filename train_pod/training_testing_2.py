...
def training(args, model, train_loader, test_loader, Loss, data_weight ,optimizer, epochs, device, num_class, checkpoint_name, event, namespace, class_threshold=0.5,swa_model=None,pretrained_state=None,fold=None):
    ...
    if pretrained_state is not None:  #c
        model	=	pretrained_state['state_dict']  #c
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch > 0:
            ...
            logging.info("wait until event is set")
            event.wait()
            ...
            model.load_state_dict(torch.load(namespace.pretrained_path)["state_dict"])  #c
            logging.info("pretrained_path model loaded successfully")
        ...
        if namespace is not None:
            ...
            torch.save({'state_dict': state['model'].state_dict()}, namespace.epoch_path)
            # torch.save({'state_dict': state['model']}, namespace.epoch_path)
        ...
        #clear event
        event.clear()
        logging.info("Clear Training Event")
